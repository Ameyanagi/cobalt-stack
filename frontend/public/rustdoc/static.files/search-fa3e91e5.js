  ...
  return {
    ...
    generics: generics.filter((gen) => {
      if (gen.bindingName !== null && gen.bindingName.name !== null) {
        ...
        bindings.set(gen.bindingName.name.toLowerCase().replace(/_/g, ''), gen.bindingName.generics)
        return false
      }
      return true
    }),
...
function getIdentEndPosition(parserState) {
  ...
  if (macroExclamation !== -1) {
    if (parserState.typeFilter === null) {
      ...
      throw ['Invalid search type: macro ', '!', ' and ', parserState.typeFilter, ' both specified']
    }
  ...
  constructor(str) {
    ...
    for (let j = 0; j < size; ++j) {
      ...
      if (is_run[j >> 3] & (1 << (j & 0x7))) {
        ...
        i += 2
        this.containers.push(new RoaringBitmapRun(runcount, u8array.slice(i, i + runcount * 4)))
        i += runcount * 4
...
class DocSearch {
  ...
  buildItemSearchType(type, paths, lowercasePaths, isAssocType) {
    ...
    if (typeof type === 'number') {
      ...
      generics = this.buildItemSearchTypeAll(type[GENERICS_DATA], paths, lowercasePaths)
  ...
  buildIndex(rawSearchIndex) {
    const buildFunctionSearchTypeCallback = (paths, lowercasePaths) => {
      const cb = (functionSearchType) => {
        ...
        if (typeof functionSearchType[INPUTS_DATA] === 'number') {
          inputs = [
            this.buildItemSearchType(functionSearchType[INPUTS_DATA], paths, lowercasePaths),
          ]
        } else {
        ...
        if (functionSearchType.length > 1) {
          if (typeof functionSearchType[OUTPUT_DATA] === 'number') {
            output = [
              this.buildItemSearchType(functionSearchType[OUTPUT_DATA], paths, lowercasePaths),
            ]
          } else {
        ...
        for (let i = 2; i < l; ++i) {
          where_clause.push(
            typeof functionSearchType[i] === 'number'
              ...
              : this.buildItemSearchTypeAll(functionSearchType[i], paths, lowercasePaths)
          )
        }
    ...
    for (const [crate, crateCorpus] of rawSearchIndex) {
      ...
      for (let i = 0; i < len; ++i) {
        ...
        const paramNames =
          paramNameString !== undefined ? paramNameString.split(',') : lastParamNames
  ...
  async execQuery(origParsedQuery, filterCrates, currentCrate) {
    ...
    const convertNameToId = (elem, isAssocType) => {
      ...
      if (
        ...
        for (const [name, { id, assocOnly }] of this.typeNameIdMap) {
          ...
          if (dist <= matchDist && dist <= maxEditDistance && (isAssocType || !assocOnly)) {
      ...
      if (
        ...
        if (elem.typeFilter === -1 && elem.normalizedPathLast.length >= 3) {
          ...
          for (const name of this.typeNameIdMap.keys()) {
            const dist = editDistance(name, elem.normalizedPathLast, maxPartDistance)
    ...
    const buildHrefAndPath = (item) => {
      ...
      if (type === 'mod') {
        ...
        displayPath = item.path + '::'
        href = this.rootPath + item.path.replace(/::/g, '/') + '/' + type + '.' + name + '.html'
      }
    ...
    const transformResults = (results, typeInfo) => {
      ...
      for (const result of results) {
        if (result.id !== -1) {
          ...
          if (typeInfo !== null) {
            obj.displayTypeSignature = this.formatDisplayTypeSignature(obj, typeInfo)
          }
          obj.href = res[1]
          out.push(obj)
    ...
    this.formatDisplayTypeSignature = async (obj, typeInfo) => {
      ...
      const writeSpecialPrimitive = (fnType, result) => {
        if (
          ...
          const [ob, sb] =
            fnType.id === this.typeNameIdOfArray || fnType.id === this.typeNameIdOfSlice
              ? ['[', ']']
              : ['(', ')']
    ...
    const sortResults = async (results, typeInfo, preferredCrate) => {
      ...
      const result_list = []
      const isReturnTypeQuery = parsedQuery.elems.length === 0 || typeInfo === 'returned'
      ...
      result_list.sort((aaa, bbb) => {
        ...
        a = Number(this.searchIndexDeprecated.get(aaa.item.crate).contains(aaa.item.bitIndex))
        b = Number(this.searchIndexDeprecated.get(bbb.item.crate).contains(bbb.item.bitIndex))
        if (a !== b) {
          return a - b
        }
        ...
        a = Number(this.searchIndexEmptyDesc.get(aaa.item.crate).contains(aaa.item.bitIndex))
        b = Number(this.searchIndexEmptyDesc.get(bbb.item.crate).contains(bbb.item.bitIndex))
        if (a !== b) {
          return a - b
        }
    ...
    function unifyGenericTypes(
      ...
      if (
        unifyFunctionTypeIsUnboxCandidate(fnType, queryElem, whereClause, mgens, unboxingDepth + 1)
      ) {
    ...
    function unifyFunctionTypeIsUnboxCandidate(
      ...
      if (fnType.id !== null && fnType.id < 0) {
        ...
        return checkIfInList(simplifiedGenerics, queryElem, whereClause, mgens, unboxingDepth)
      }
      return false
    }
    ...
    const checkType = (row, elem, whereClause, mgens, unboxingDepth) => {
      ...
      if (
        ...
        return unifyFunctionTypes([row], [elem], whereClause, mgens, () => true, unboxingDepth)
      }
    }
    ...
    function checkPath(contains, ty) {
      ...
      pathiter: for (let i = length - clength; i >= 0; i -= 1) {
        ...
        for (let x = 0; x < clength; ++x) {
          ...
          if (Math.floor((p.length - c.length) / 3) <= maxPathEditDistance && p.indexOf(c) !== -1) {
    ...
    function handleArgs(row, pos, results) {
      ...
      const tfpDist = compareTypeFingerprints(row.id, parsedQuery.typeFingerprint)
      if (tfpDist === null) {
        return
      }
        ...
        const handleNameSearch = (id) => {
          ...
          if (parsedQuery.literalSearch) {
            ...
            addIntoResults(
              ...
              editDistance(row.normalizedName, elem.normalizedPathLast, maxEditDistance),
              pathDist,
              maxEditDistance
            )
          }
        ...
        for (let i = 0, nSearchIndex = length; i < nSearchIndex; ++i) {
          ...
          if (tfpDist !== null) {
            ...
            if (in_args) {
              ...
              const maxDist =
                results_in_args.size < MAX_RESULTS ? tfpDist + 1 : results_in_args.max_dist
            ...
            if (returned) {
              ...
              const maxDist =
                results_returned.size < MAX_RESULTS ? tfpDist + 1 : results_returned.max_dist
    ...
    const ret = createQueryResults(sorted_in_args, sorted_returned, sorted_others, parsedQuery)
    await handleAliases(ret, parsedQuery.userQuery.replace(/"/g, ''), filterCrates, currentCrate)
    await Promise.all(
...
function getFilterCrates() {
  ...
  if (elem && elem.value !== 'all crates' && window.searchIndex.has(elem.value)) {
    return elem.value
  }
  return null
}
...
async function addTab(array, query, display) {
  ...
  const output = document.createElement(array.length === 0 && query.error === null ? 'div' : 'ul')
  if (array.length > 0) {
...
function makeTabHeader(tabNb, text, nbElems) {
  ...
  if (searchState.currentTab === tabNb) {
    return (
      '<button class="selected">' + text + '<span class="count">' + fmtNbElems + '</span></button>'
    )
  }
...
async function showResults(results, go_to_first, filterCrates) {
  ...
  if (results.query.error !== null) {
    ...
    output +=
      '<div id="search-tabs">' + makeTabHeader(0, 'In Names', results.others.length) + '</div>'
    currentTab = 0
